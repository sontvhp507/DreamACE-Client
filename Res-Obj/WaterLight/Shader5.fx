...
float4 WaterPS(VS_OUTPUT input) : COLOR
{
    ...
    // Reflection — sky with horizon haze, modulated by scene light
    ...
    waterBody += float3(0.008, 0.020, 0.015) * NdotL * LightColor;
    ...
    color += float3(0.04, 0.06, 0.07) * slopeBright * LightColor;

    // Specular — visible highlights on wave slopes
    float spec1 = pow(max(dot(R, lightDir), 0.0), 384.0);
    color += spec1 * 12.0 * float3(0.90, 0.93, 0.98) * LightColor;

    // Broader glow
    float spec2 = pow(max(dot(R, lightDir), 0.0), 28.0);
    color += spec2 * 0.10 * float3(0.65, 0.72, 0.82) * LightColor;
