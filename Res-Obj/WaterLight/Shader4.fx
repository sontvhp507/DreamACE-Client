...
// Procedural sky with horizon haze, modulated by scene light
float3 getSkyColor(float3 e, float3 lightCol)
{
    e.y = max(e.y, 0.0);
    float3 skyZenith  = float3(0.15, 0.30, 0.60);
    float3 skyHorizon = float3(0.55, 0.65, 0.78);
    float grad = pow(1.0 - e.y, 3.0);
    return lerp(skyZenith, skyHorizon, grad) * lightCol;
}
...
float4 WaterPS(VS_OUTPUT input) : COLOR
{
    ...
    float3 reflected = getSkyColor(R, LightColor);
    ...
    float3 sss = float3(0.05, 0.80, 0.70) * sssVdotH * heightFrac * LightColor;
