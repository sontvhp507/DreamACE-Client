// Water_ClearOcean.fx - Clear bright ocean with 5-octave billow noise, with scene lighting
...
float4 WaterPS(VS_OUTPUT input) : COLOR
{
    ...
    // Bright clear sky reflection, modulated by scene light
    ...
    float3 reflected = lerp(skyZenith, skyHorizon, skyGrad) * LightColor;
    ...
    waterBody += float3(0.02, 0.05, 0.04) * NdotL * LightColor;
    ...
    color += float3(0.05, 0.08, 0.10) * slopeBright * LightColor;

    // Light foam at crests
    float crestFoam = pow(saturate(waveHeight * 1.8) * saturate(slope * 4.0), 2.0);
    color = lerp(color, float3(0.80, 0.85, 0.90) * LightColor, crestFoam * 0.5);

    // Specular — sun highlight
    float spec = pow(max(dot(R, lightDir), 0.0), 128.0);
    color += spec * 3.0 * float3(1.0, 0.97, 0.92) * LightColor;

    // Broader glow
    float spec2 = pow(max(dot(R, lightDir), 0.0), 20.0);
    color += spec2 * 0.08 * float3(0.7, 0.75, 0.85) * LightColor;
