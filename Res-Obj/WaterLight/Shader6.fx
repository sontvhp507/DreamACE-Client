// Water_Seascape.fx - Classic ocean with 4-octave directional FBM with scene lighting
...
float3 getSkyColor(float3 e, float3 lightCol)
{
    ...
    float sc = 1.0 - ey;
    return float3(sc * sc, sc, 0.6 + sc * 0.4) * 0.95 * lightCol;
}
...
float4 WaterPS(VS_OUTPUT input) : COLOR
{
    ...
    float3 refracted = waterBase
        + seaDiffuse(normal, lightDir, 80.0) * float3(0.3, 0.38, 0.25) * 0.10 * LightColor;
    ...
    color += spec * specNorm * LightColor;
