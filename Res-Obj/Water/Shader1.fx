...
float4 WaterPS(VS_OUTPUT input) : COLOR
{
    ...
    float ey = skyE.y;
    float sc = 1.0 - ey;
    float3 reflected = float3(sc * sc, 1.0 - ey, 0.6 + (1.0 - ey) * 0.4) * 0.99;

    // Bright teal base with green-cyan diffuse
    float3 refracted = float3(0.08, 0.22, 0.25)
        + seaDiffuse(normal, lightDir, 80.0) * float3(0.5, 0.85, 0.65) * 0.25;
    ...
    color += float3(0.5, 0.85, 0.65) * max(waveH - 0.5, 0.0) * 0.15 * atten;
